...
const ADDITION_CNT = '追加されたページ数';
const APPROVAL_CNT = '承認されたページ数';
const UNAPPROVAL_CNT = '未承認のページ数';

const i = ' '; // Scrapboxでインデントを示す文字

const addBackquote = (text) => `\`${text}\``;
const removeBackquote = (text) => text.split('').filter(char => char !== '\`').join('');
...
if (prevLogs.length > 0) {
  ...
  const prevTotalCnt = Boolean(found)
    ? parseInt(removeBackquote(found).split(`${TOTAL_CNT}: `)[1])
    : 0;
  const additionCnt = pageList.count - prevTotalCnt;
  
  body.push(
    `${i}${addBackquote(`${TOTAL_CNT}: ${pageList.count}`)}`,
    `${i}${addBackquote(`${ADDITION_CNT}: ${additionCnt}`)}`,
    `${i}${addBackquote(`${APPROVAL_CNT}: 0`)}`,
    `${i}${addBackquote(`${UNAPPROVAL_CNT}: 0`)}`,
  );

} else {
  body.push(
    `${i}${addBackquote(`${TOTAL_CNT}: ${pageList.count}`)}`,
    `${i}${addBackquote(`${ADDITION_CNT}: ${pageList.count}`)}`,
    `${i}${addBackquote(`${APPROVAL_CNT}: 0`)}`,
    `${i}${addBackquote(`${UNAPPROVAL_CNT}: 0`)}`,
  );
}
